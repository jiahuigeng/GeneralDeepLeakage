...
def group_consistency(dummy_data, group_dummy_data):
    mean_group_data = sum(group_dummy_data) / len(group_dummy_data)
    return torch.norm(dummy_data - mean_group_data, p=2)

method_config = {
    "dlg": {"distance": "l2", "optlabel": True},
    "cpl": {"distance": "l2", "lm":0.01, "optlabel": True},
    "inverting": {"distance": "cosine", "tv": 0.01, "optlabel": True},
    "gradinversion": {"distance": "l2", "group_num": 5, "tv": 1e-4, "bn": 0.1, "l2": 1e-6, "gc": 1e-2, "optlabel": False},
    "gdlg": {"distance":"l2", "tv":1e3, "clip":2e3, "scale": 2e3, "optlabel": False},
}

def gradient_clousure(optimizer, dummy_data, original_dy_dxs, original_nets, dummy_label, label_pred, method, criterion, dummy_group, tv_alpha=1000, clip_alpha=100, scale_alpha=100, l2_alpha=0):
    torch.manual_seed(7)
    def closure():
        optimizer.zero_grad()
        ...
        lm_loss = 0
        gc_loss = group_consistency(dummy_data, dummy_group)
        ...
        for net, dy_dx in zip(original_nets, original_dy_dxs):
            ...
            # if method in ["dlg", 'inverting', 'cpl']:
            #     dummy_loss = - torch.mean(torch.sum(torch.softmax(dummy_label, -1) * torch.log(torch.softmax(pred, -1)), dim=-1))
            # elif method in ['gradinversino', 'gdlg']:
            #     dummy_loss = criterion(pred, label_pred)
            if method_config[method]["optlabel"]:
                dummy_loss = - torch.mean(torch.sum(torch.softmax(dummy_label, -1) * torch.log(torch.softmax(pred, -1)), dim=-1))
            else:
                dummy_loss = criterion(pred, label_pred)
            ...
            for dummy_gl, gl in zip(dummy_dy_dx, dy_dx):
                if method_config[method]["distance"] == "l2":
                    grad_diff += ((dummy_gl - gl) **2).sum()
                elif method_config[method]["distance"] == "cosine":
                    grad_diff += 0



            if 'lm' in method_config[method]:
                grad_diff += method_config[method]['lm'] * lm_loss

            if 'l2' in method_config[method]:
                grad_diff += method_config[method]['l2'] * l2_loss

            if 'clip' in method_config[method]:
                grad_diff += method_config[method]['clip'] * clip_loss

            if 'scale' in method_config[method]:
                grad_diff += method_config[method]['scale'] * scale_loss

            if 'gc' in method_config[method]:
                grad_diff += method_config[method]['gc'] * gc_loss

            if 'tv' in method_config[method]:
                grad_diff += method_config[method]['tv'] * tv_loss

            if 'bn' in method_config[method]:
                grad_diff += method_config[method]['bn'] * bn_loss
...
def main(args):
    ...
    for idx_net in range(num_exp):
        ...
        for method in attack_list: # model
            ...
            group_dummy_data = []
            group_dummy_label = []
            group_dummy_optimizer = []

            if 'group_num' in method_config[method]:
                for _ in range(method_config[method]['group_num']):
                    tmp_data, tmp_label, optimizer = None, None, None
                    group_dummy_data.append(tmp_data)
                    group_dummy_label.append(tmp_label)
