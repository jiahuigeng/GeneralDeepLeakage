    ...
    for idx_net in range(num_exp):
        ...
        for method in attack_list: # model
            ...
            if args.mode == "gradients":
                ...
                out, _ = copy_net(gt_data)
                y = criterion(out, gt_label)
                # myview = make_dot(out, params=dict(copy_net.named_parameters()), show_attrs=True, show_saved=True)
                # myview.format ='png'
                # myview.directory = "data"
                # myview.view()
                tmp_dy_dx = torch.autograd.grad(y, copy_net.parameters())
                original_dy_dxs.append(list((_.detach().clone() for _ in tmp_dy_dx)))
                original_nets.append(copy_net)
                for i in range(1, args.multi_steps):
                    net = train_steps(net, gt_data, gt_label, device, args)
                    # copy_net = net.clone()#copy.deepcopy(net)
                    tmp_net = net.state_dict()
                    copy_net = get_model(channel=channel, hidden=hidden, num_classes=num_classes, args=args)
                    copy_net.load_state_dict(tmp_net)
                    out, _ = copy_net(gt_data)
                    y = criterion(out, gt_label)
                    tmp_dy_dx = torch.autograd.grad(y, copy_net.parameters())
                    original_dy_dxs.append(list((_.detach().clone() for _ in tmp_dy_dx)))
            ...
            for idx_step in range(args.multi_steps):
                dummy_data_copy = copy.deepcopy(dummy_data)

                optimizer = torch.optim.LBFGS([dummy_data_copy, ], lr=lr)
                if args.opt == "adam":
                    optimizer = torch.optim.Adam([dummy_data_copy, ], lr=lr)

                label_pred = []
                if method == 'dlg':
                    optimizer = torch.optim.LBFGS([dummy_data_copy, dummy_label], lr=lr)
                    ...
                    tmp_min = torch.sum(original_dy_dxs[idx_step][-2], dim=-1)
                    dummy_z, dummy_o = original_nets[idx_step](dummy_data_copy)
                ...
                original_net, original_dy_dx = original_nets[idx_step], original_dy_dxs[idx_step]
                if args.infer_mode in ["both", "data"]:
                    ...
                    logger.info("multistep:"+ str(idx_step))
                    ...
                    for iters in range(Iteration):
                        # 由于
                        closure = gradient_clousure(optimizer, dummy_data_copy, [original_dy_dx], [original_net], dummy_label, label_pred,
                        ...
                        mses.append(torch.mean((dummy_data_copy - gt_data) ** 2).item())

                        if iters % int(Iteration / 5) == 0:
                            current_time = str(time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime()))
                            logger.info('%s, %d, loss = %.8f, mse = %.8f' % (current_time, iters, current_loss, mses[-1]))
                            result_imgs.append(dummy_data_copy.detach().clone().cpu())
                            history.append([tp(dummy_data_copy[imidx].cpu()) for imidx in range(bs)])
                    ...
                    gradients_diff = get_grad_diff(dummy_data_copy, original_dy_dxs, net, label_pred, criterion)
                    ...
                    if args.skip:
                        ...
                        try:
                            gt_data_copy, gt_label_copy, dummy_data_copy, label_pred_copy, total_sim, label_acc, pairs = image_align(
                                gt_data, gt_label, dummy_data_copy, label_pred, "psnr")
                        except:
                            continue
                    ...
                    if not args.skip:
                        saver.save_result_imgs(result_imgs, gt_label.detach().clone().cpu(), start_idx, "_"+method+"_ms"+str(idx_step))
                    ...
                    saver.save_result_imgs(result_imgs, gt_label.detach().clone().cpu(), start_idx, "_"+method+"_ms"+str(idx_step)+"_align")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='lenet', choices=['lenet', 'resnet', 'mlp', 'vbmlp'],
