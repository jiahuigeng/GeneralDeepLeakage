...
method_config = {
    ...
    "cpl": {"distance": "l2", "lm":0.01, "optlabel": False},
    "gs": {"distance": "cosine", "tv": 0.01, "optlabel": False},
    "gradinversion": {"distance": "l2", "group_num": 5, "tv": 1e-4, "bn": 0.1, "l2": 1e-6, "gc": 1e-2, "optlabel": False},
    "gdlg": {"distance":"l2", "tv":1e-4, "clip":0, "scale": 0, "optlabel": False},
}
...
def gradient_clousure(optimizer, dummy_data, original_dy_dxs, original_nets, dummy_label, label_pred, method, criterion, tv_alpha=1000, clip_alpha=100, scale_alpha=100, l2_alpha=0):
    ...
    def closure():
        ...
        for net, dy_dx in zip(original_nets, original_dy_dxs):
            ...
            if method == "dlg":
                ...
                if 'lm' in method_config[method]:
                    lm_loss = label_matching(pred, label_pred)
                dummy_loss = criterion(pred, label_pred)
            dummy_dy_dx = torch.autograd.grad(dummy_loss, net.parameters(), create_graph=True)
            ...
            if method == "dlg":
                restore_loss += grad_diff

            elif method == "cpl":
                restore_loss += grad_diff + lm_loss * method_config[method]['lm']

            elif method == "gs":
                restore_loss += grad_diff + total_variation(dummy_data) * method_config[method]['tv']

            elif method == "gradinversion":
                restore_loss += grad_diff \
                    #             + gc_loss * method_config[method]['gc'] + bn_loss * method_config[method]['bn'] + \
                    # l2_loss * method_config[method]['l2']

            elif method == "gdlg":
                restore_loss += grad_diff + total_variation(dummy_data) * method_config[method]['tv']
                                # + scale_loss * method_config[method]['scale'] + \
                    # clip_loss * method_config[method]['clip']
        ...
        return restore_loss / len(original_dy_dxs)
...
def main(args):
    ...
    for idx_net in range(num_exp):
        ...
        attack_list = ["dlg", "cpl", "gs"] # "dlg", "gradinversion", "dlg", "cpl", "gs", "gradinversion",
        # if bs > num_classes:
        ...
        for method in attack_list: # model
            ...
            elif method in ["gradinversion", "cpl", "gs"]:
